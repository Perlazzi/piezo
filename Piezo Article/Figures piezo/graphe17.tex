...
\begin{tikzpicture}[scale=0.8,line cap=round,line join=round,>=triangle 45,x=1.0cm,y=1.0cm]
  \node (root) at (0,0) {$[\octa^-]\circledcirc [\DD_m\oplus \ZZ_2^c]$};

  \node (Bas1) at (-3.5,-2.5) {};
  \node (Bas2) at (-1,-2.5)   {\begin{tabular}{l} $[\1],[\ZZ_2^{e_i}]$ \\ $[\ZZ_2^{ec_l-}],[\ZZ_3^{vt_i}]$\end{tabular}};
  \node (Bas3) at (2.5,-2.5) {\begin{tabular}{l}  $[\1]$\\$[\ZZ_2^{e_i}], [\ZZ_2^{ec_l-}]$\end{tabular}};
  ...
  \node (Bas11) at (-3.5-1.5,-5) {\begin{tabular}{l} $[\1],[\ZZ_2^{e_i}]$\\$[\ZZ_4^{e_i-}] ,[\ZZ_2^{ec_l-}]$ \\ $[\DD_4^h],[\DD_2^v]$\end{tabular}};
  \node (Bas12) at (-3.5+1.5,-5) {\begin{tabular}{l}  $[\1],[\ZZ_2^{e_i}]$\\$ [\ZZ_2^{ec_l-}],[\DD_2]$\end{tabular}};
